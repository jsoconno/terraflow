...
class TerraformDocumentation:
    ...
    def _get_schema_data(self):
        if self.type == "provider":
            ...
            return self.schema.get_resource_schema(
                self.namespace, self.provider, self.kind
            )
        elif self.type == "data":
            return self.schema.get_data_schema(self.namespace, self.provider, self.kind)
        else:
            raise ValueError(
                "Invalid type. Must be one of 'provider', 'resource', or 'data'."
            )
    ...
    def _get_docs_url(self):
        if self.type == "provider":
            ...
            raise ValueError(
                "Invalid scope. Must be one of 'provider', 'resource', or 'data'."
            )
    ...
    def _get_docs_text(self):
        ...
        if self.use_cache:
            ...
            documentation_dir = os.path.join(
                DOCUMENTATION_DIR, self.namespace, self.provider, self.version
            )
    ...
    def _get_attribute_metadata(
        ...
        for attribute, attribute_schema in attributes.items():
            ...
            attribute_description = get_resource_attribute_description(
                self.text, attribute, block_hierarchy
            )
            ...
            is_output = (
                True if (not is_required and not is_optional) or id == "id" else False
            )
